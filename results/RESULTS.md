...
- **Réplication sur 2ᵉ architecture — RWKV-7 1.5B (`diag_rule_transfer.py`, 31 graines)** : le même transfert voyelle/consonne marche sur RWKV-7 (delta rule généralisée, état récurrent lu **sans modif**) — plafond 0,782 ; FULL **0,173** ; Δ vs no-transfer +0,173 **p<10⁻⁴**, vs random +0,093 **p=0,009**, vs wrong-label +0,129 **p=0,002**. ⇒ le mécanisme n'est **pas spécifique à DeltaNet**. (Caveat : implémentation fla RWKV-7 signalée non bit-exacte ; effet comportemental cohérent entre architectures.) Cadrage : 2 instances du **même** checkpoint → généralité d'architecture, pas cross-checkpoint.
