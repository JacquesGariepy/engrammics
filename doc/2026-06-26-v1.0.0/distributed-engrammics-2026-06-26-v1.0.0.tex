...
\paragraph{Reading.}
The pre-specified claims (gradient-free transfer, rank dose-response, and captured-key subspace governance) hold in the DeltaNet backend at both $1.3$B and $2.7$B, and what circulates is not only a memorized table: the fast state can carry at least one simple generalizing rule (\S\ref{sec:rule}), which replicates on a second architecture (RWKV-7, there only borderline against the random control). What does \emph{not} survive intact is interference-free superposition and surgical forgetting; we localize the cause to shared key directions and confirm it by construction (Table~\ref{tab:disjoint}). The same harness that supports the algebra in the controlled regime exposes its limits in the DeltaNet backend and points to the missing precondition, disjoint key allocation, for future work.
...
\paragraph{Outlook: the architectural bet.}
Everything here is gated on a model exposing a single additive recurrent
fast-weight state, today a minority of deployed systems. We are explicit that the
wager motivating the work---that such states grow more common---is a \emph{bet},
not a finding: no measured claim depends on it. The case is concrete. Selective
state-space and gated linear-attention models (Mamba and its successors
\cite{gu2023mamba}, gated DeltaNet \cite{yang2024gdn}, RWKV-7 \cite{peng2025rwkv7})
are trained at growing scale for the long-context regime in which a fixed-size
additive state is cheaper than a growing KV cache; and \emph{hybrid} stacks that
interleave a few such layers with softmax attention
\cite{lieber2024jamba,de2024griffin} are the plausible bridge to mainstream
deployment, since a mostly-softmax model carrying even a handful of additive-state
layers already exposes a partial substrate into which an engram could in principle be
inscribed, gated within an authorized key subspace, and partially forgotten by the
algebra of \S\ref{sec:method}---a possibility this work does not test. If that share grows, the primitive demonstrated
here---gradient-free skill exchange, subspace-gated consent, and auditable partial
erasure---becomes a substrate-level operation on whichever layers hold such a
state, not a curiosity confined to research checkpoints; if it does not, the
method stays confined to linear-attention models. We surface the conditional so
the reader can price it independently; the empirical results stand either way.
...
\bibitem{irie2022} K.~Irie, I.~Schlag, R.~Csord\'as, and J.~Schmidhuber. A modern self-referential weight matrix that learns to modify itself. In \emph{ICML}, 2022. arXiv:2202.05780.
...
\bibitem{lieber2024jamba} O.~Lieber, B.~Lenz, et al. Jamba: a hybrid Transformer-Mamba language model. arXiv:2403.19887, 2024.
\bibitem{de2024griffin} S.~De, S.~L.~Smith, et al. Griffin: mixing gated linear recurrences with local attention for efficient language models. arXiv:2402.19427, 2024.
\end{thebibliography}
